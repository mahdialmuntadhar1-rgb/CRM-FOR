...
function getWhatsAppPhone(normalizedPhone: string | null): string | null {
...
async function fetchSourceData(): Promise<SourceBusiness[]> {
  console.log('📥 Fetching data from SOURCE database (with pagination)...');
  
  const allData: SourceBusiness[] = [];
  let offset = 0;
  const limit = 1000;
  let hasMore = true;
...
function transformData(businesses: SourceBusiness[]): TargetContact[] {
  ...
  const transformed = businesses.map(business => {
    ...
    if (!contact.normalized_phone) return false; // Skip null phones
...
async function upsertBatch(contacts: TargetContact[], batchNumber: number, totalBatches: number): Promise<number> {
