...
function normalizePhone(phone) {
    if (!phone)
        return { normalized: '', whatsapp: '', raw: '' };
...
async function selectDiverseBusinesses(client) {
...
async function generateCaption(business, genAI) {
...
function generateFallbackCaption(business) {
...
// Generate posts
async function generatePosts(businesses, genAI) {
...
async function insertPosts(client, posts) {
    ...
    for (let i = 0; i < posts.length; i += batchSize) {
        ...
        const preparedBatch = batch.map(post => {
            ...
            if (!existingColumns.length || existingColumns.includes('business_id'))
                prepared.business_id = post.business_id;
            if (!existingColumns.length || existingColumns.includes('businessId'))
                prepared.businessId = post.business_id;
            if (!existingColumns.length || existingColumns.includes('display_name'))
                prepared.display_name = post.display_name;
            if (!existingColumns.length || existingColumns.includes('businessName'))
                prepared.businessName = post.display_name;
            if (!existingColumns.length || existingColumns.includes('caption_ar'))
                prepared.caption_ar = post.caption_ar;
            if (!existingColumns.length || existingColumns.includes('caption'))
                prepared.caption = post.caption_ar;
            if (!existingColumns.length || existingColumns.includes('content'))
                prepared.content = post.caption_ar;
            if (!existingColumns.length || existingColumns.includes('caption_en'))
                prepared.caption_en = post.caption_en;
            if (!existingColumns.length || existingColumns.includes('image_url'))
                prepared.image_url = post.image_url;
            if (!existingColumns.length || existingColumns.includes('imageUrl'))
                prepared.imageUrl = post.image_url;
            if (!existingColumns.length || existingColumns.includes('image_prompt'))
                prepared.image_prompt = post.image_prompt;
            if (!existingColumns.length || existingColumns.includes('category'))
                prepared.category = post.category;
            if (!existingColumns.length || existingColumns.includes('governorate'))
                prepared.governorate = post.governorate;
            if (!existingColumns.length || existingColumns.includes('raw_phone'))
                prepared.raw_phone = post.raw_phone;
            if (!existingColumns.length || existingColumns.includes('normalized_phone'))
                prepared.normalized_phone = post.normalized_phone;
            if (!existingColumns.length || existingColumns.includes('whatsapp_phone'))
                prepared.whatsapp_phone = post.whatsapp_phone;
            if (!existingColumns.length || existingColumns.includes('post_style'))
                prepared.post_style = post.post_style;
            if (!existingColumns.length || existingColumns.includes('featured'))
                prepared.featured = post.featured;
            // Set defaults for required fields that might be missing
            if (!existingColumns.length || existingColumns.includes('isVerified'))
                prepared.isVerified = false;
            if (!existingColumns.length || existingColumns.includes('likes'))
                prepared.likes = 0;
            if (!existingColumns.length || existingColumns.includes('businessAvatar'))
                prepared.businessAvatar = null;
            // Always set published timestamp
            if (!existingColumns.length || existingColumns.includes('published_at'))
                prepared.published_at = new Date().toISOString();
            if (!existingColumns.length || existingColumns.includes('createdAt'))
                prepared.createdAt = new Date().toISOString();
            if (!existingColumns.length || existingColumns.includes('created_at'))
